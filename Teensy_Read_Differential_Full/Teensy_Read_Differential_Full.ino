...
void setup() {
  ...
  adc->enablePGA  (64, ADC_0) ; //can be 1, 2, 4, 8, 16, 32 or 64 // just gives me a flat 0 will amplify signals of less that 1.2V only in differential mode (see examples). Only for Teensy 3.1/3.2.
...
void loop() {
  ...
  value = (uint16_t)adc->analogReadDifferential(A10,A11); // the unsigned is necessary for 16 bits, otherwise values larger than 3.3/2 V are negative!
